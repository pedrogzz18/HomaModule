 ...
 * [max size ratio] [Gigabytes per second]
 */
int main (int argc, char**argv)
{
  ...
  if (argc < 2) {
    fprintf(stderr, "Usage: %s workload [max message length] [min bucket frac]"
            " [max size ratio] > <file name>", argv[0]);
  }
  if (argc > 2) {
  ...
  /**
   * Convert average size to bits, then divide by gbps and round up to get us,
   * then multiply by 2 because server and client share thread
   */
  double interval_us = (std::ceil( (generator.get_mean() * 8.0) / gbps)) * 2;
  fprintf(stderr,"%.0f", interval_us);
}